...
export default function SignUp()
{
    ...
    const signup = async (evt) =>
    {
        ...
        await fetch(`${ server }createUser?email=${ email }&password=${ pass }`).then(res => res.json()).then(async (data) =>
        {
            console.log(data);
            if (data.success)
            {
    ...
    return <div >
        ...
        <form style={styles.formOnly} onSubmit={(evt) => signup(evt)}>
            {error ? (<><label style={{ color: 'red', fontWeight: 800, fontSize: 20 }}>Error : All fields are mandatory</label><br /></>) : (<></>)}
